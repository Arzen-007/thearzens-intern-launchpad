...
function CatalogEditor({ record, connected }: { record?: ManagedResource; connected: boolean }) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="flex max-h-[calc(100vh-8rem)] flex-col gap-0 overflow-hidden border-cyan-300/30 bg-[#071018] p-0 text-cyan-50 sm:max-w-3xl">
        <DialogHeader className="shrink-0 border-b border-cyan-300/15 px-6 py-5">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-6 py-4">
          ...
          {!connected && <p className="mt-4 rounded border border-amber-300/30 bg-amber-300/10 p-3 text-sm text-amber-100">Publishing is locked until the private GitHub App is connected. No draft is stored locally or sent to GitHub.</p>}
        </div>
        <DialogFooter className="shrink-0 border-t border-cyan-300/15 px-6 py-4"><Button type="button" onClick={() => upsert.mutate({ resource: draft })} disabled={!canSubmit || upsert.isPending} className="bg-cyan-300 text-slate-950 hover:bg-cyan-200">{upsert.isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Committing</> : <><Github className="mr-2 h-4 w-4" />Commit catalog change</>}</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  );
}
